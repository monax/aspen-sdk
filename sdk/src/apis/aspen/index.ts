...
export class ApiClient {
  ...
  public async request<T extends keyof pathMeta>(
    operationId: T,
    params: t.TypeOf<pathMeta[T]['paramsSchema']>,
    query: t.TypeOf<pathMeta[T]['querySchema']>,
    body: t.TypeOf<pathMeta[T]['bodySchema']>,
    options: { baseUrl?: string; token?: string } = {},
    extraHeaders: Record<string, string> = {},
  ): Promise<t.TypeOf<pathMeta[T]['responseSchema']>> {
    const meta = pathMeta[operationId];

    let url = this.combineURLs(options.baseUrl ?? this._baseUrl, meta.url);

    const queryType = meta.querySchema as t.Type<t.TypeOf<pathMeta[T]['querySchema']>>;
    const queryData = queryType.name !== 'null' ? queryType.encode(query) : query;

    // replace params in url
    const paramsType = meta.paramsSchema as t.Type<t.TypeOf<pathMeta[T]['paramsSchema']>>;
    if (paramsType.name !== 'null') {
      const paramsData = paramsType.encode(params) || {};
      Object.entries(paramsData).forEach(([k, v]) => {
        url = url.replace(`:${k}`, (v as string).toString());
      });
    }

    const headers: Record<string, string> = {
      Accept: meta.hasBinaryResponse ? '*/*' : 'application/json',
    };

    const bodyType = meta.bodySchema as t.Type<t.TypeOf<pathMeta[T]['bodySchema']>>;
    let formData: FormData | undefined = undefined;

    if (meta.isBodyFormData) {
      headers['Content-Type'] = 'multipart/form-data';
      formData = this.getFormData(body as unknown as Record<string, unknown>);
    } else if (bodyType.name !== 'null') {
      body = bodyType.encode(body);
    }

    // Widen type to avoid compilation error when paths contains no auth of a particular type
    const security: readonly string[] = meta.security;
    if (!!security.find((s) => s === 'BearerAuth')) {
      headers.Authorization = `Bearer ${options.token ?? this._token}`;
    }

    if (!!security.find((s) => s === 'OptionalAuth')) {
      headers.Authorization = `Bearer ${options.token ?? this._token}`;
    }

    if (!!security.find((s) => s === 'API Token')) {
      headers.Authorization = `${options.token ?? this._token}`;
    }
    ...
    if (meta.hasBinaryResponse) {
      config.responseType = 'blob';
    }

    const response = await axios.request<unknown, AxiosResponse<t.TypeOf<pathMeta[T]['responseSchema']>>>(config);

    const responseType = meta.responseSchema as t.Type<t.TypeOf<pathMeta[T]['responseSchema']>>;

    if (responseType) {
      // the above if won't properly handle nulls which return an empty
      // string from the API which makes the parse below puke since
      // an empty string definitely doesn't equal a null
      if (responseType.name === 'null') {
        return null;
      }
      try {
        return parse(responseType, response.data);
      } catch (err) {
        // Dont want to break the app if we cant parse the response
        // Ideally this should never happen, and we should eventually remove this try/catch
        console.error('Failed to parse payload from ' + operationId, err);
  ...
  getFormData(data: Record<string, unknown>): FormData | undefined {
    if (data) {
